...
//translate([-50,0,28+7.5])rotate([0,90,0])color([0.1,0.1,0.1,0.4]) import("../assembly/1515_1000mm.stl");
//translate([57, 35, 8]) rotate([180, 0, 30]) % nema17();

////////// RENDER EXTRUDER //////////////////////////////////////////////////////////////
//wade(hotend_mount=groovemount, legacy_mount=0);
...
wade(hotend_mount=bowden_mount, legacy_mount=0);
...
////////// RENDER IDLER /////////////////////////////////////////////////////////////////////
//translate([-15,-35,16.20])		// POSITIONING
//	rotate([0,-90,0])
//		wadeidler();
...
// 		@jsturm making some optics - filling a gap on the upper arm
			...
			translate(idler_fulcrum){
				...
				difference(){
					...
					cylinder(r=idler_hinge_r+1,h=10);
				}
				rotate(-15)
...
//*******************************************************************************************
      ...
      {
  			//The base.
			 translate([-base_leadout,0,0]) cube([base_length,base_thickness,wade_block_depth]);

			 // Round the ends of the base
			 translate([base_length-base_leadout,base_thickness/2,0]) cylinder(r=base_thickness/2,h=wade_block_depth);
			 translate([-base_leadout,base_thickness/2,0]) cylinder(r=base_thickness/2,h=wade_block_depth);
		  }
//*******************************************************************************************			
...
module block_holes(legacy_mount=false){
	...
	render()
	difference(){
		translate([-1,0,0])	cube([block_bevel_r+1,block_bevel_r+1,wade_block_depth+2]);
		translate([block_bevel_r,0,0])	cylinder(r=block_bevel_r,h=wade_block_depth+2);
	}

	// Round the top front corner.
	translate ([-base_leadout-base_thickness/2,-1-base_thickness/2,wade_block_depth-block_bevel_r])
	render()
	difference(){
		translate([-1,0,0])	cube([block_bevel_r+1,base_thickness+2,block_bevel_r+1]);
		rotate([-90,0,0])	translate([block_bevel_r,0,-1])	cylinder(r=block_bevel_r,h=base_thickness+4);
	}

	// Round the bottom front corner.
	translate ([-base_leadout-base_thickness/2,-1-base_thickness/2,0])
	render()
	difference(){
		translate([-1,0,-1])	cube([block_bevel_r+1,base_thickness+2,block_bevel_r+1]);
		rotate([-90,0,0])	translate([block_bevel_r,-block_bevel_r,-1])	cylinder(r=block_bevel_r,h=base_thickness+4);
	}
	
		// Round the top back corner.
	translate ([base_length-base_leadout+base_thickness/2-block_bevel_r,
		-1-base_thickness/2,wade_block_depth-block_bevel_r])
	render()
	difference(){
		translate([0,0,0]) cube([block_bevel_r+1,base_thickness+2,block_bevel_r+1]);
		rotate([-90,0,0])	translate([0,0,-1])	cylinder(r=block_bevel_r,h=base_thickness+4);
	}
	
	// Round the bottom back corner.
	translate ([base_length-block_bevel_r/2,-1-base_thickness/2,0])
	...
	cylinder(r=m3_diameter/2,h=idler_short_side-2*idler_hinge_width-0.5,center=true);

	translate(idler_fulcrum+[0,0,idler_short_side/2-idler_hinge_width-1])
	cylinder(r=m3_nut_diameter/2+0.25,h=1);

	//Rounded cutout for idler hinge.
	render()
	translate(idler_fulcrum)
	difference(){
		cylinder(r=idler_hinge_r+0.5,h=idler_short_side+0.5,center=true);
...
*/
      ...
      // Mounting holes
      translate([-filament_feed_hole_offset-12,-31-base_thickness/2,0]) {
        cylinder(r=m3_diameter/2,h=wade_block_depth*3,center=true);
        translate([0,0,wade_block_depth-20/2-20+3.5])cylinder(r=3,h=20,center=true); //
      }  
      translate([base_length-filament_feed_hole_offset-24,-31-base_thickness/2,0]) {
        cylinder(r=m3_diameter/2,h=wade_block_depth*3,center=true);
        translate([0,0,wade_block_depth-20/2-20+3.5])cylinder(r=3,h=20,center=true);
      }
...
// screwhead resp. 3mm washer
        translate([0, 0, 0]) {
            cylinder(r = 3.7, h = 3.0, center = false);}
// schaft
        translate([0, 0, 3.0+layer_thickness]) {
            cylinder(r = 1.75, h = 5.05, center = false);
        }
    }
...
//%cube([12,4,5],center=true);
	...
	difference(){
		union(){
			...
			translate(idler_fulcrum)
			rotate([0,0,-30]){
				cylinder(h=idler_short_side,r=idler_hinge_r,center=true);
		...
		translate(idler_axis+[2+guide_height,+idler_long_side-idler_long_bottom-guide_length/2,0]){
			...
			translate([-7/2,0,0])
			rotate([90,0,0])
			cylinder(h=guide_length+4,r=3.5/2,center=true);
		}
		...
		translate(idler_fulcrum){
			cylinder(h=idler_short_side-2*idler_hinge_width,
				r=idler_hinge_r+0.5,center=true);
			rotate(-30)
		...
		translate(idler_fulcrum)
		rotate(360/12)
		cylinder(h=idler_short_side+2,r=m3_diameter/2-0.1,center=true);
...
//		rotate([0,0,30])
		...
		translate(idler_axis+[2-idler_height,0,0]){
			...
			rotate([0,90,0]){
				cylinder(r=idler_mounting_hole_diameter/2,h=idler_height+2);
			...
			difference(){
				...
				cylinder(h=idler_height+4,r=idler_corners_radius,center=true);
			}
		}
	}
}
...
module b608(h=8){
	translate([0,0,h/2]) cylinder(r=hole_for_608/2,h=h,center=true);
}
		...
		rotate(90,[1,0,0])
			translate([hole*(extruder_recess_d/2-1.5),3+1.5,-wade_block_depth/2-1]){
				translate([0,0,3.25+layer_thickness])
					cylinder(r=1.5,h=wade_block_depth+2);
				translate([0,0,1.75])
					nut_trap(m3_wrench, 3);
			}
	}
